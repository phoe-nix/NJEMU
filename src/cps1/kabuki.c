		...
		int base_addr, int length, int swap_key1, int swap_key2, int addr_key, int xor_key)
{
	...
	{
		/* decode opcodes */
		select = (addr + base_addr) + addr_key;
		dest_op[addr] = bytedecode(src[addr], swap_key1, swap_key2, xor_key, select);

		/* decode data */
		select = ((addr + base_addr) ^ 0x1fc0) + addr_key + 1;
		dest_data[addr] = bytedecode(src[addr], swap_key1, swap_key2, xor_key, select);
	}
}
