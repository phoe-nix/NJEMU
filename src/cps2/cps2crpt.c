...
static void cps2_decrypt(const UINT32 *master_key, UINT32 upper_limit)
{
	...
	UINT32 length = memory_length_user1;
	UINT16 *dec = (UINT16 *)memory_region_user1;
	UINT32 i, percent = 0, prev_percent = 0;
	UINT32 key1[4];
	...
	for (i = 0; i < 0x10000; ++i)
	{
		...
		if ((i & 0xff) == 0)
		{
			percent = i*100/0x10000;
			if (percent != prev_percent)
			{
				msg_printf(TEXT(DECRYPTING_x), percent);
	...
	msg_printf(TEXT(DECRYPTING_100));

	m68000_init();
	m68000_set_encrypted_range(0, length - 1, dec);
}
...
static const struct game_keys keys_table[] =
{
	...
	{ "megamn2a", { 0x3f148a2b,0xd6790a15 }, 0x100000 },	// 0C80 0164 7101  cmpi.l  #$01647101,D0
	...
	{ "sfa2",     { 0x1bbf3d96,0x8af4614a }, 0x100000 },	// 0C80 3039 9783  cmpi.l  #$30399783,D0
	...
	{ "spf2h",    { 0x706a8750,0x7d0fc185 }, 0x040000 },	// 0C80 3039 9819  cmpi.l  #$30399819,D0
